...
const zipPath = path.join(__dirname, `/unGTasks-${platform}.zip`);
const distPath = path.join(__dirname, '/dist');
console.log(`Checking unGTasks-${platform}.zip...`);
fs.rm(zipPath, async (err) => {
  if (!err) {
    console.log(`  Delete unGTasks-${platform}.zip`);
  }
  ...
  try {
    ...
    console.log(`Creating unGTasks-${platform}.zip file...`);
